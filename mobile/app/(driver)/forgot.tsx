/**
 * Driver "Forgot password" screen — mirrors the rider forgot screen but
 * uses the driver-specific endpoint. The reset link in the email opens a
 * web page at https://turanelitelimo.com/driver-reset-password?token=...
 * so the driver can choose a new password from any browser (handles cases
 * where they don't have the mobile app installed on the device that
 * receives the email).
 */
import { useState } from "react";
import { View, Text, StyleSheet, Pressable, ScrollView, ImageBackground } from "react-native";
import { useRouter } from "expo-router";
import { ChevronLeft, Mail, Check, Briefcase } from "lucide-react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import Button from "@/components/Button";
import Input from "@/components/Input";
import { colors, assets } from "@/theme";
import { driverForgotPassword } from "@/api";
...
export default function DriverForgotPassword() {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      await driverForgotPassword(email.trim().toLowerCase());
      setSent(true);
    } catch {
    ...
    <View style={s.root}>
      ...
      <SafeAreaView style={s.safe}>
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
          <Pressable testID="driver-forgot-back" onPress={() => router.back()} style={s.backBtn}>
          ...
          {sent ? (
            <View style={s.card}>
              ...
              <Pressable
                testID="driver-forgot-back-to-signin"
                onPress={() => router.replace("/(driver)/auth")}
                style={s.primaryBtn}
              >
            ...
            <View style={s.card}>
              ...
              <Text style={s.sub}>
                Enter the email dispatch issued you. We'll send you a one-time reset link.
              </Text>

              <Input
                testID="driver-forgot-email"
                label="Driver email"
              ...
              <Pressable
                testID="driver-forgot-back-to-signin-alt"
                onPress={() => router.back()}
                hitSlop={6}
                style={s.linkBtn}
              >
...
const s = StyleSheet.create({
  ...
  heroSpacer: { height: 50 },
  badge: { alignSelf: "flex-start", flexDirection: "row", gap: 6, alignItems: "center", paddingHorizontal: 12, paddingVertical: 5, borderRadius: 999, borderWidth: 1, borderColor: "rgba(212,175,55,0.3)", backgroundColor: "rgba(212,175,55,0.08)", marginBottom: 16 },
